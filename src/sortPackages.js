...
function sortPackagesMultipleDependencies (packageList) {
  const packageDetailsList = packageList.map(packageListItem => {
    const [ packageName, dependencies ] = packageListItem.split(': ')
    return { packageName, dependencies: dependencies.split(', ') }
  })
  console.log(packageDetailsList)
  ...
  while (packageDetailsList.length) {
    ...
    // Add the package and all it's dependencies in the correct order to the list.
    const dependencyTree = getDependencyTree(packageDetails.packageName, packageDetails.dependencies, packageDetailsList)
    orderedPackageNames.push(...dependencyTree) // TODO: find a way to flatten the tree before pushing it
  }
...
packageDetailsList: an array of objects with the structure { packageName, dependencies }
usedDependencies: an array a string package names that have already been used in the current tree
Returns: an array containing all dependencies down the chain needed to include the package found in packageDetails, as well as the package itself
Throws: if a circular dependency is detected
 */
function getDependencyTree(packageName, dependencies, packageDetailsList, usedDependencies = []) {
  // If there are no dependencies, return an array with just the package name itself.
  ...
  for (let i = 0; i < dependencies.length; i++) {
    ...
    // Add the dependency to the list and go deeper.
    const dependencyTree = module.exports.getDependencyTree(dependencyDetails.packageName, dependencyDetails.dependency, packageDetailsList, [ ...usedDependencies, packageName ])
    dependencyTrees.push(dependencyTree)
  }

  // TODO: merge dependency trees before returning.
